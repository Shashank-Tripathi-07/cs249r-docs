...
- **`CHANGELOG.md` is several releases behind the actual shipped version.** Its newest entry is `[0.1.10]`, dated 2026-04 and marked "planned," but `pyproject.toml`'s `version` field (the value `tinytorch.__version__` actually reads at import time) is `0.1.13` as of this document, meaning at least three point releases have shipped with no changelog entry at all.
- **`INSTRUCTOR.md` documents two `tito module status` flags that don't exist.** Its "Tracking Progress" section shows `tito module status --student student_id` and `tito module status --export class_progress.csv` as working commands for checking individual or class-wide progress. The real `status` subparser (`tito/commands/module/workflow.py`) takes no arguments at all beyond the bare command; both examples would fail with an argparse "unrecognized arguments" error if an instructor actually typed them. There is no per-student or CSV-export progress view anywhere in the codebase.
- **`quarto/tito/troubleshooting.qmd` uses a package path that doesn't exist.** Several of its examples (`from tinytorch.nn import Linear`, `ls tinytorch/nn/layers.py`) reference a `tinytorch.nn` submodule; no `nn` directory exists anywhere under `tinytorch/tinytorch/`. The real path for that same class is `tinytorch.core.layers`, matching every other document and the actual `MODULE_EXPORT_CHECKS` mapping in `tito/commands/milestone.py`.
- **`quarto/tito/overview.qmd`'s "Complete Command Reference" is missing two entire top-level command groups and one real subcommand.** `tito package` (`reset`, `nbdev`) and `tito olympics` don't appear anywhere in the table, despite both being real, registered commands (see the command table earlier in this document). The table's "Module Commands" section also omits `tito module path`, a real subcommand (`path_parser` in `workflow.py`) used by editor/IDE integrations to resolve a module's file path.
...
- **A five-PR batch merged 2026-08-12 fixed a cluster of small but genuinely student-facing bugs, found by driving the whole `tito` CLI surface by hand rather than reading the source.** All five were verified live (not just read) against a fresh install before being fixed: `797a2ed901` fixed six separate places across `commands/community.py` and `core/submission.py` that told a student to run `tito login`, a command that has never existed (the real one is `tito community login`; running the printed instruction literally fails with "'login' is not a valid command"). `f262404573` fixed `tito system health`'s final "Module Status" section, which imported and reran the unrelated `InfoCommand` instead of showing per-module status, printing a second full System Details table (Python version, disk space, memory) in place of anything module-related. `a19da98b1d` fixed `tito benchmark baseline` crashing with a raw, unhandled `EOFError` traceback on any non-interactive invocation (no piped stdin), plus a second bug in the same function where the submission-creation block was indented as a sibling of `if submit:` rather than nested inside it, so answering "no" to the submit prompt crashed with `NameError` on variables only assigned in the "yes" branch. `b67c8382a9` fixed the milestone achievement panel showing the identical "What makes this special" text for every milestone, including "Every gradient: YOUR autograd" for Milestones 01 and 02, which only require modules 1 through 3 (forward pass only, no autograd module at all). `897bd4cc9c` fixed `tito milestone info` and the "What's Next" preview both rendering a milestone's year twice ("Perceptron (1958) (1958)"), since `milestone['name']` already includes the year and both call sites appended it a second time.
