...
- **design-grammar** (full detail: [`design-grammar/ci-workflows.md`](design-grammar/ci-workflows.md)) has no CI validation at all, corrected here from an earlier version of this doc that claimed it piggybacked on `book-validate-dev.yml`. Checked directly: that workflow's only mention of "design-grammar" is a stale re-trigger comment, not a real path filter or check. `auto-label.yml` references it only for label routing. The project's own `npm run validate` script exists but nothing in CI currently runs it.
...
## 6. CI incidents, verified against real fix commits

Two of these were found by tracing an actual failing run end to end. The rest were mined from `git log --full-history -- .github/workflows/` (2,226 commits touch this directory) filtered to real fix commits, each one below is a genuine, dated, merged fix, not a hypothetical. Grouped by pattern rather than by date, because the same mistake recurring independently across projects is the more useful thing to know than a flat chronological list.

### Pattern A: concurrency group too coarse, dispatch collides with push

This exact bug class hit three different projects independently, at three different times, which makes it the single most repeated mistake in this repo's CI history.

- **StaffML, 2026-05-04 (`2a61ece3f5`)**: `staffml-validate-dev.yml` and `staffml-validate-vault.yml` both derived their concurrency group from `${{ github.workflow }}`, which resolves to the *caller's* name inside a `workflow_call`, not the callee's own name. Called from the same parent (`staffml-preview-dev.yml`), both reusable workflows collapsed into one group, and `cancel-in-progress` silently killed whichever queued a few seconds later, despite both actually passing. Fixed with a literal, workflow-identifying string plus `${{ github.head_ref || github.run_id }}` instead of `${{ github.workflow }}`.
- **TinyTorch and Labs, 2026-05-17 (`df88ff7a72`)**: both workflows grouped concurrency only on `github.ref`, so a manual `workflow_dispatch` (or, for TinyTorch, a `workflow_call` from `publish-live`) landing on the same SHA as a push would cancel the push run, showing red on a commit that was actually healthy. Fixed by switching to the same `head_ref || run_id` pattern already in use by kits, mlsysim, site, slides, and instructors, meaning those five had already gotten this right by the time TinyTorch and Labs hit the bug.
- **Book, 2026-05-01 (`2b1f7ef1ed`)**: identical root cause, `book-validate-${{ github.ref }}` as the group key meant a manual dispatch on the same SHA as a push collided with it. Fixed by folding `${{ github.event_name }}` into the group key so dispatch and push runs never share a slot.

If a new reusable or dispatchable workflow is ever added to this repo, checking its concurrency group against this exact pattern (`github.workflow` inside a `workflow_call`, or a bare `github.ref` with both `push` and `workflow_dispatch` triggers) is worth doing before it ships, given it has now independently bitten three projects.

### Pattern B: deploy shipping ahead of, or despite, a failed validation

- **StaffML, 2026-05-01 (`6ddb82a71b`)**: `staffml-preview-dev.yml` and its validate workflows were both independently push-triggered on `dev`, running in parallel with no gate between them, so a deploy could ship on a SHA that Validate (Dev) or Validate (Vault) had failed on. Restructured into the current `validate-dev` (uses) + `validate-vault` (uses) + `build` + `deploy` shape with `needs:` on all three, the design documented in section 3's StaffML entry.
- **StaffML, 2026-06-16 (`b4f74be474`)**: a narrower version of the same class of problem, `staffml-preview-dev.yml`'s path filter (which drives the README badge) didn't include `interviews/vault-cli/**`, even though its own called `validate-vault` job lints vault-cli. A real fix (PR #1879) landed, `validate-vault` went green, but `preview-dev` never re-ran because nothing in its trigger paths had changed, leaving the badge stuck showing the prior failing run indefinitely.

### Pattern C: CI catching drift between what code assumes and what actually changed

- **Labs, 2026-06-15 (`cbdaf6f5fe`, issue #1859)**: the WASM browser smoke test built and staged only the `mlsysim` engine wheel, not the `mlsysbook-labs` helper wheel every lab's Cell 0 also installs. `micropip` 404'd fetching the missing wheel and Pyodide raised `BadZipFile: File is not a zip file` for every single lab in the smoke test.
- **Labs, 2026-05-29 (`4dbfe7152a`)**: the smoke test's own import statement (`from mlsysim.core.engine import Engine`) referenced a pre-refactor internal path. A taxonomy refactor had moved `Engine` and re-exported it at the package top level, and nothing but this CI step still used the old path, so it broke with `ModuleNotFoundError` purely as CI debt, not a real product bug.
- **MLSys·im, 2026-05-27 (`c874a7c69b`)**: CI ran Python 3.11, which has a genuine circular-import bug in its import machinery when relative imports re-enter a partially-initialized parent package. Rather than working around it with import restructuring, the fix bumped CI to Python 3.12, where PEP 690 handles the case correctly, and reverted the registry module back to clean relative imports.
- **MLSys·im, 2026-05-26 (`782f763259`)**: `pre-commit`'s fallback dependency set (used when its normal environment resolution can't reach the network) was missing `pytest`, breaking the `mlsysim-check-registry-gates` check specifically in CI. A reminder that a "fallback" dependency list is a second, easy-to-forget source of truth that can silently drift from the real one.

### Pattern D: permissions and shell-quoting mistakes

- **StaffML, 2026-05-01 (`39336c4d9c`)**: CodeQL flagged `staffml-validate-vault.yml` for having no explicit `permissions` block at all, meaning it defaulted to the repo-wide token policy, broader than the job (checkout, install, lint, test) actually needed. Tightened to `contents: read`. Closed two separate CodeQL alerts.
- **StaffML, 2026-04-28 (`e153729057`)**: `staffml-publish-live.yml`'s smoke-test step had a botched heredoc conversion, opened with `python3 -c "..."`, closed with a `PY` heredoc terminator, and the inner Python f-strings used double quotes that silently closed the outer shell string six lines in. Bash then choked on the next `(` and exited 2 before a single line of the intended Python ever ran. A second bug rode along in the same step: two `open()` calls referenced a `root` variable that was never actually defined.
- **StaffML, 2026-04-29 (`e0d1d61edc`, issue #1600)**: the deploy validation step checked for flat `<page>.html` output paths, but `next.config.mjs` sets `trailingSlash: true`, so Next.js actually emits `<page>/index.html`. The check aborted deploys on every single push despite the build itself being completely correct, until it was rewritten to check the same path shape `staffml-validate-dev.yml` already checked correctly.
- **Repo-wide, 2026-05-01 (`440c82651a`, PR #1622)**: the contributor auto-crediting workflow's comment-posting step returned 403 on every fork PR, because the default `pull_request` trigger delivers a read-only `GITHUB_TOKEN` to fork PRs regardless of what permissions the workflow declares. Switched to `pull_request_target`, which runs in the base-repo context with the real token, the fix is safe specifically because the workflow never checks out fork code and only ever feeds plain text (title, body, file paths) to its LLM classification step, never anything fork-controlled.

### Pattern E: a check that CI never actually runs, so drift accumulates silently

Different failure shape from A through D, these aren't misconfigured checks, they're checks that don't exist at all, so the underlying problem can sit unnoticed for an unknown period before anyone happens to hit it by hand.

- **MLPerf EDU, confirmed still unfixed as of 2026-08-10**: `mlperf-edu-validate-dev.yml`'s test suite depends on `matplotlib` (via `tools/generate_paper_figures.py`), but `matplotlib` was never added to `mlperf-edu/pyproject.toml` or `uv.lock`. Every PR touching `mlperf-edu`, regardless of what it actually changes, fails `Tests, Audit, and Package Portability` with `ModuleNotFoundError: No module named 'matplotlib'`. A second, independent bug rides in the same test suite: a workload-registry count assertion hardcoded to 9 when the registry actually has 14 entries. Neither is caught by anything, because nothing re-validates that the test suite's own dependencies or hardcoded assumptions still match reality; both were only found by tracing red CI runs on unrelated Dependabot PRs back to their actual cause. See [`mlperf-edu/ci-workflows.md`](mlperf-edu/ci-workflows.md) for the fuller writeup, including that the maintainer currently merges through this by direct override rather than blocking on a fix.
- **StaffML, 2026-08-10 (PR #1981)**: `eslint` had been silently broken for every contributor since a prior Dependabot bump (`eslint@10.4.0` outran the peer range `eslint-config-next`'s nested `eslint-plugin-react` actually supports), because `staffml-validate-dev.yml` has never included a lint step. `npm run lint` crashed outright for anyone who ran it locally; CI simply never ran it, so nothing ever turned red. Once lint was restored, it immediately surfaced three real, previously-invisible issues in the same PR. See [`staffml/ci-workflows.md`](staffml/ci-workflows.md).

The shared lesson: "CI is green" only means what the workflow file actually checks, not what a contributor might assume it checks. If a sub-project's test suite has an implicit dependency (a package import, a hardcoded count, a path) that nothing re-verifies against the source of truth, it can drift for months with a fully green badge the whole time.

### A cluster worth knowing about but not fully detailed here

A run of terse commits between 2026-06-20 and 2026-06-23 (`02996dcb1c` Linux container TeX Live preflight, `855803df04` a wrong Cloudflare purge token, `52906be2d8` the publish chain continuing after a failed deploy, `0b98b740eb` TinyTorch PDFs being wiped on a site-only deploy) reads like a real multi-day incident around the release pipeline rather than isolated bugs, but the commit messages themselves are too terse to reconstruct the full story from git log alone. Worth a closer look if you are about to touch `infra-cloudflare-purge.yml` or any `publish-live.yml`'s site-only path, since that's exactly the code these commits were patching under pressure.
